...
create table assets (id int, type enum('source', 'sink', 'storage', 'pump', 'pipe', 'recycling_plant'), issue_count int default 0, property varchar(255), value text, latitude double, longitude double, parent_aggregation_id int, primary key(id),  foreign key (parent_aggregation_id) references aggregations(id));
